    ...
    serialRows.forEach(sr => {
        ...
        Array.from(sr.querySelectorAll('strong')).forEach(str => {
            ...
            if (label.includes('seriale')) {
                tipo = 'Seriale';
                const m = parentText.match(/Seriale[\s:]+(.+?)(?=\s*Stato|$)/i);
                if (m) seriale = m[1].trim();
            }
            if (label.includes('lotto')) {
                tipo = 'Lotto';
                const m = parentText.match(/Lotto[\s:]+(.+?)(?=\s*Stato|$)/i);
                if (m) seriale = m[1].trim();
            }
        ...
        if (!seriale) {
            ...
            else if (mL) { seriale = mL[1].trim(); tipo = 'Lotto'; }
        }
...
// ===== MODIFICA: estraiRighe restituisce rowsSeriali e rowsLotti separati =====
function estraiRighe() {
    ...
    const outSeriali = [header];
    const outLotti   = [header];
    ...
    console.log(' Totale lotti esportati:', totalLotti);
    console.log(' Ubicazione Prelievo finale:', ubicazionePrelievo);

    return { rowsSeriali: outSeriali, totalSeriali, rowsLotti: outLotti, totalLotti };
}
...
// ===== MODIFICA: downloadCSV scarica file separati per Seriali e Lotti =====
async function downloadCSV() {
    await waitForSerials();
    const { rowsSeriali, totalSeriali, rowsLotti, totalLotti } = estraiRighe();

    if (totalSeriali === 0 && totalLotti === 0) {
        alert('Nessun seriale o lotto trovato!');
        return;
    }
    ...
    // --- FILE SERIALI ---
    if (totalSeriali > 0) {
        downloadExcel(rowsSeriali, `${base} - SERIALI.xlsx`);

        const csvSeriali = toCSV(rowsSeriali);
        GM_download({
            url: "data:text/csv;charset=utf-8," + encodeURIComponent(csvSeriali),
            name: `${base} - SERIALI.csv`,
            saveAs: true
        });
        ...
        GM_xmlhttpRequest({
            ...
            data: JSON.stringify({ fileName: `${base} - SERIALI.csv`, content: base64Seriali }),
            onload: function (response) {
                try {
                    const data = JSON.parse(response.responseText);
                    if (data.success) console.log("✅ SERIALI caricati su Drive:", data.url);
                    else console.error("❌ Errore Drive SERIALI:", data.error);
                } catch (err) {
                    console.log("⚠️ Risposta non JSON (SERIALI), upload presumibilmente completato.");
                }
            },
            onerror: function (err) {
                console.log("⚠️ Errore rete SERIALI verso Apps Script ignorato:", err);
            }
        });
    }
    ...
    // --- FILE LOTTI ---
    if (totalLotti > 0) {
        downloadExcel(rowsLotti, `${base} - LOTTI.xlsx`);

        const csvLotti = toCSV(rowsLotti);
        GM_download({
            url: "data:text/csv;charset=utf-8," + encodeURIComponent(csvLotti),
            name: `${base} - LOTTI.csv`,
            saveAs: true
        });
        ...
        GM_xmlhttpRequest({
            ...
            url: scriptUrl,
            headers: { "Content-Type": "application/json" },
            data: JSON.stringify({ fileName: `${base} - LOTTI.csv`, content: base64Lotti }),
            onload: function (response) {
                try {
                    const data = JSON.parse(response.responseText);
                    if (data.success) console.log("✅ LOTTI caricati su Drive:", data.url);
                    else console.error("❌ Errore Drive LOTTI:", data.error);
                } catch (err) {
                    console.log("⚠️ Risposta non JSON (LOTTI), upload presumibilmente completato.");
                }
            },
            onerror: function (err) {
                console.log("⚠️ Errore rete LOTTI verso Apps Script ignorato:", err);
            }
        });
    }

    alert(`Estrazione completata!\nSeriali esportati: ${totalSeriali}\nLotti esportati: ${totalLotti}`);
}
...
function addSearchBar() {
    if (document.getElementById('grn-search-bar')) return;

    if (!document.getElementById('grn-search-styles')) {
        const s = document.createElement('style');
        s.id = 'grn-search-styles';
        s.textContent = `
            .grn-row-hidden {
                display: none !important;
            }
            .grn-row-filter-match {
                box-shadow: inset 5px 0 0 #0d6efd,
                            inset 0 0 0 2000px rgba(13,110,253,0.07) !important;
            }
            .grn-row-scroll-match {
                box-shadow: inset 5px 0 0 #28a745,
                            inset 0 0 0 2000px rgba(40,167,69,0.07) !important;
            }
            .grn-row-scroll-active {
                box-shadow: inset 5px 0 0 #ffc107,
                            inset 0 0 0 2000px rgba(255,193,7,0.15) !important;
            }
            #grn-search-bar {
                resize: both;
                overflow: auto;
    ...
    handle.id = 'grn-search-handle';
    handle.innerHTML = '⠿ Cerca nelle righe';
    ...
    handle.addEventListener('mousedown', (e) => {
        ...
        dragOffsetY = e.clientY - bar.getBoundingClientRect().top;
    ...
    [
        { value: 'generic',   label: '🔤 Testo generico 🔻' },
        { value: 'rif',       label: '📋 Riferimento (Rif) 🔻' },
        { value: 'posizione', label: '📍 Posizione 🔻' },
        { value: 'pn',        label: '🔢 Part Number 🔻' },
        { value: 'seriale',   label: '#️⃣ Seriale / Lotto 🔻' },
    ...
    modeBtn.id = 'grn-search-mode';
    modeBtn.dataset.mode = 'filter';
    modeBtn.textContent = '🔽 Filtra';
    ...
    modeBtn.onclick = () => {
        if (modeBtn.dataset.mode === 'filter') {
            modeBtn.dataset.mode = 'highlight';
            modeBtn.textContent = '🎯 Scroll';
            modeBtn.style.background = '#6f42c1';
            modeBtn.style.borderColor = '#6f42c1';
        } else {
            modeBtn.dataset.mode = 'filter';
            modeBtn.textContent = '🔽 Filtra';
            modeBtn.style.background = '#fd7e14';
            modeBtn.style.borderColor = '#fd7e14';
        }
        clearSearch();
    ...
    counter.id = 'grn-search-counter';
    counter.style.fontSize = '12px';
    counter.style.color = '#555';
    counter.style.minWidth = '50px';
    counter.style.textAlign = 'right';

    actionRow.appendChild(btnSearch);
    ...
    btnPrev.textContent = '⬆ Prec';
    const btnNext = document.createElement('button');
    btnNext.textContent = '⬇ Succ';
    ...
    navLabel.id = 'grn-search-nav-label';
    navLabel.style.fontSize = '12px';
    navLabel.style.color = '#555';
    ...
    function getRowText(li, type) {
        const data = getDataFromLi(li);
        switch(type) {
            case 'rif':       return (data.riferimento + ' ' + data.riferimentoOrdine).toLowerCase();
            case 'posizione': return (data.posizione || '').toLowerCase();
            case 'pn':        return (data.pn || '').toLowerCase();
            case 'seriale':   return data.serials.map(s => s.seriale).join(' ').toLowerCase();
            case 'articolo':  return (data.articolo || '').toLowerCase();
            default:          return (li.innerText || li.textContent || '').toLowerCase();
        }
    }

    function clearRowClasses(li) {
        li.classList.remove(
            'grn-row-hidden',
            'grn-row-filter-match',
            'grn-row-scroll-match',
    ...
    function executeSearch(query) {
        ...
        righe.forEach(li => {
            clearRowClasses(li);
            const match = getRowText(li, type).includes(q);
            if (mode === 'filter') {
                if (match) { li.classList.add('grn-row-filter-match'); currentResults.push(li); }
                else li.classList.add('grn-row-hidden');
            } else {
                if (match) { li.classList.add('grn-row-scroll-match'); currentResults.push(li); }
            }
        });

        counter.textContent = `${currentResults.length} trovati`;
        counter.style.color = currentResults.length > 0 ? '#28a745' : '#dc3545';

        if (mode === 'highlight' && currentResults.length > 0) {
    ...
    function scrollToResult(idx) {
        ...
        currentIndex = idx;
        currentResults.forEach((li, i) => {
            li.classList.remove('grn-row-scroll-active', 'grn-row-scroll-match');
            li.classList.add(i === idx ? 'grn-row-scroll-active' : 'grn-row-scroll-match');
        });
        currentResults[idx].scrollIntoView({ behavior: 'smooth', block: 'center' });
        navLabel.textContent = `${idx + 1} / ${currentResults.length}`;
    }
    ...
    // ===== EVENT LISTENERS =====
    btnSearch.onclick = () => executeSearch(input.value);
    btnReset.onclick = () => { input.value = ''; clearSearch(); };
    btnPrev.onclick = () => scrollToResult(currentIndex - 1);
    btnNext.onclick = () => scrollToResult(currentIndex + 1);
    ...
    input.addEventListener('input', () => {
        ...
        debounceTimer = setTimeout(() => {
            if (input.value.trim().length >= 2) executeSearch(input.value);
            else if (input.value.trim() === '') clearSearch();
        }, 350);
    });
}
    ...
    function addExportAndPrintUI() {
        ...
        // Pulsante apri ricerca (floating trigger)
if (!document.getElementById('grn-search-trigger')) {
    const trigger = document.createElement('button');
    trigger.id = 'grn-search-trigger';
    trigger.textContent = '🔍';
    trigger.title = 'Apri barra di ricerca righe';
    ...
    trigger.onclick = () => {
        const bar = document.getElementById('grn-search-bar');
